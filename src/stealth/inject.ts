 ...
 */
export async function injectStealth(client: any): Promise<void> {
  if (process.env.SURFAGENT_STEALTH === '0') return;
  try {
    const { Page } = client;
    if (!Page || typeof Page.addScriptToEvaluateOnNewDocument !== 'function') return;
    ...
    if (process.env.SURFAGENT_DEBUG === '1') {
      console.error('[surfagent] stealth inject failed:', (e as Error).message);
    }
  }
}
