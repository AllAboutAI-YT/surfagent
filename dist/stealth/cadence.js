 ...
 */
function parseRange(env, fallback) {
    if (!env)
        return fallback;
...
export async function sleepJitter(rangeEnv = 'SURFAGENT_CLICK_JITTER_MS', fallback = [80, 400]) {
 ...
 */
export async function humanMouseTo(client, targetX, targetY) {
    if (!client || !client.Input || typeof client.Input.dispatchMouseEvent !== 'function')
        return;
    const { Input } = client;
